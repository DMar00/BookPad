...
<c:set var="followings" scope="request" value='${user.followings}'/>
<div id="followings-list">
    <h4 style="text-align:center;margin-bottom:20px;">Followings</h4>
    <div class="row" id="container-stories">
        <c:choose>
            <c:when test="${followings.size()==0}">
                <h4 class="no-result">Non segui nessun utente!</h4>
            </c:when>
            <c:otherwise>
                <c:forEach items="${followings}" var="user">
